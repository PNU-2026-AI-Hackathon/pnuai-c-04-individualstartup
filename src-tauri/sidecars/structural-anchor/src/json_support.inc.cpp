...
double get_number(const Json& json, const std::string& key, double default_value = 0.0) {
    const Json* value = json.get(key);
    return value && value->is_number() ? value->number_value : default_value;
}

bool get_bool(const Json& json, const std::string& key, bool default_value = false) {
    const Json* value = json.get(key);
    return value && value->is_bool() ? value->bool_value : default_value;
}
