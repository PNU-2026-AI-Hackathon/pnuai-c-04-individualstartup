...
> 자연어로 아이디어를 설명하면, 편집 가능하고 신뢰 가능한 3D CAD를 만드는 AI 기반 데스크톱 CAD 워크스페이스

CADGen-AX는 CAD 경험이 적은 사용자도 텍스트로 요구사항을 전달해 정밀한 3D 모델을 생성하고, 수정·검증·저장까지 할 수 있도록 돕는 데스크톱 프로토타입입니다.
...
- 세션, 대화, 리비전, 검증 배치와 산출물을 저장하여 관리
- 최종 모델을 STL 포맷으로 내보내기
...
2. 모델 계획 커밋, 소스 적용, 런타임 진단, 최종화의 단계별 진행을 확인합니다.
4. 실행 중 취소하거나 실패한 작업을 다시 시도할 수 있습니다.
...
- 구조, DFM, VLM 및 종합 보고서를 실행 내역에서 펼쳐볼 수 있습니다.
  - 구조 검사는 생성된 3D 모델의 Self-intersection, Manifold 여부 등 기하적·위상적 타당성을 평가합니다. 
  - VLM은 구조·구성요소·비율을 각각 0~3점으로 평가하며, 각 항목 2점 이상이면서 합계 7점 이상이어야 통과합니다.
  - DFM은 Slicer를 활용하여 3D printer 사양 및 G-code 생성으로 3D 프린팅 가능성을 평가합니다.
...
[![Video Label](https://youtu.be/THNZbOqVQsM?si=MKESyMzIVFIMEyv2)
